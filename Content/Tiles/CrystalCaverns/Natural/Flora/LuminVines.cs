...
namespace AerovelenceMod.Content.Tiles.CrystalCaverns.Natural.Flora
{
    internal class LuminVines : ModTile
    {
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            Texture2D glow = (Texture2D)ModContent.Request<Texture2D>("AerovelenceMod/Content/Tiles/CrystalCaverns/Natural/Flora/LuminVines_Glow");
        ...
        public override bool TileFrame(int i, int j, ref bool resetFrame, ref bool noBreak)
        {
            ...
            if (!tileBelow.HasTile && !tileBelow.HasTile && tileBelow.TileType != TileType<CavernStoneTile>() && tileBelow.LiquidAmount > 250)
            {
                Framing.GetTileSafely(i, j).TileFrameX = 36;
                Framing.GetTileSafely(i, j).TileFrameY = (short)(Main.rand.Next(2) * 18);
            }
            else if (!tileAbove.HasTile && !tileAbove.HasTile && tileAbove.TileType != TileType<CavernStoneTile>() && tileBelow.LiquidAmount > 250)
            {
            ...
            if (!tileAbove.HasTile && !tileAbove.HasTile && tileAbove.TileType != TileType<CavernStoneTile>() && tileBelow.LiquidAmount > 250 && !tileBelow.HasTile && !tileBelow.HasTile && (tileBelow.TileType != TileType<CavernStoneTile>() && tileBelow.LiquidAmount > 250))
            {
                Framing.GetTileSafely(i, j).TileFrameX = 0;
